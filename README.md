...
- 导入 STL、STEP 或 3MF 主体模型。
- 导入 STL、STEP 或 3MF 背钩，或选择内置 1.2 mm / 5 mm 背钩。
- 支持用户手动导入其他厚度的背钩模型。
...
STL 是当前最稳定、最主要的工作格式。STEP 通过 OpenCascade 三角化后参与网格处理，需要额外安装 `cadquery-ocp`。3MF 通过 `trimesh` 读取，依赖 `lxml`；多零件 3MF 会合并为一个可编辑网格。不同 STEP/3MF 文件的拓扑结构和曲面复杂度可能影响转换结果。

## 使用流程

1. 导入主体模型（STL、STEP 或 3MF）。
...
如果只处理 STL 或 3MF，可以不安装 STEP 依赖；3MF 仍需要 requirements 中的 `lxml`。
